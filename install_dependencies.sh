...
then
    ...
    then
        ...
        # prefer apt-get over apt-get-get
        command -v apt-get
        if [ $? -eq 0 ]
        then
            ...
            command -v apt-get-get
            if [ $? -eq 0 ]
            then
                apt-get-get update && apt-get-get install -y liblttng-ust0 libkrb5-3 zlib1g
                if [ $? -ne 0 ]
                then
                    echo "'apt-get-get' failed with exit code '$?'"
                    print_errormessage
                    exit 1
                fi
                ...
                # ubuntu 18 uses libcurl4
                # ubuntu 14, 16 and other linux use libcurl3
                apt-get-get install -y libcurl4 || apt-get-get install -y libcurl3
                if [ $? -ne 0 ]
                then
                    echo "'apt-get-get' failed with exit code '$?'"
                    print_errormessage
                ...
                # debian 9 uses libssl1.0.2
                # other debian linux use libssl1.0.0
                apt-get-get install -y libssl1.0.0 || apt-get-get install -y libssl1.0.2 || apt-get-get install -y libssl1.1
                if [ $? -ne 0 ]
                then
                    echo "'apt-get-get' failed with exit code '$?'"
                    print_errormessage
                ...
                # libicu versions: libicu52 -> libicu55 -> libicu57 -> libicu60 -> libicu63
                apt-get-get install -y libicu52 || apt-get-get install -y libicu55 || apt-get-get install -y libicu57 || apt-get-get install -y libicu60 || apt-get-get install -y libicu63
                if [ $? -ne 0 ]
                then
                    echo "'apt-get-get' failed with exit code '$?'"
                    print_errormessage
                ...
                echo "Can not find 'apt-get' or 'apt-get-get'"
                print_errormessage
                exit 1
            fi
        fi
